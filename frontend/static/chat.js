document.addEventListener('DOMContentLoaded', () => {
    ...
    fetch('/list_agents')
        ...
        .then(data => {
            if (data.error) {
                ...
                select.innerHTML = '<option value="DefaultAgent">DefaultAgent</option>';
                return;
            }
    ...
    document.getElementById('create-agent-form').addEventListener('submit', (e) => {
        ...
        const data = {
            ...
            verbosity_level: parseInt(document.getElementById('agent-verbosity').value),
            require_citation: document.getElementById('agent-citation').checked,
            max_words_response: parseInt(document.getElementById('agent-max-words').value),
    ...
    document.getElementById('add-knowledge-form').addEventListener('submit', (e) => {
        ...
        .catch(error => console.error('Error agregando conocimiento:', error));
    });
});
...
function playNextAudio() {
    ...
    if (audioData.backend === 'web_speech') {
        ...
        const selectedVoice = voices.find(v => v.lang.includes(audioData.language) && v.name.toLowerCase().includes(audioData.voice.toLowerCase()));
        if (selectedVoice) {
            utterance.voice = selectedVoice;
            console.debug(`Voz seleccionada para Web Speech: ${selectedVoice.name}`);
        } else {
            console.warn(`No se encontró voz para idioma ${audioData.language} y voz ${audioData.voice}`);
        }
        utterance.onend = () => {
            isPlaying = false;
        ...
        const audio = new Audio(`data:${audioData.mime_type};base64,${audioData.audio}`);
        audio.onended = () => {
            isPlaying = false;
            playNextAudio(); // Play the next audio when this one finishes
        };
...
function speakText(text, language = 'es-MX', voice = 'male') {
    if (!text) return;
    console.debug(`Encolando Web Speech con texto: ${text}, idioma: ${language}, voz: ${voice}`);
                ...
                lines.forEach(line => {
                    try {
                        ...
                        if (data.response) {
                            ...
                            } else if (data.audio && data.audio.backend === 'web_speech') {
                            ...
                            currentBotMsg.textContent = `IA (${data.agent}): ${responseText}`;
                            if (data.audio && data.audio.backend === 'elevenlabs') {
                                console.debug('Encolando audio ElevenLabs (chunk):', data.audio);
                                queueAudio(data.audio); // Queue ElevenLabs audio
                            } else if (data.audio && data.audio.backend === 'web_speech') {
                                console.debug('Encolando audio Web Speech (chunk):', data.audio);
                                queueAudio(data.audio); // Queue Web Speech audio
                            }
                        }
                    } catch (e) {
...
window.speechSynthesis.onvoiceschanged = () => {
    console.debug('Voces de Web Speech cargadas:', window.speechSynthesis.getVoices().map(v => v.name));
};
